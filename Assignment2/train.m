...
learning_rate = 0.0001; %0.1;  % Learning rate; default = 0.1.
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    if mod(m, show_validation_CE_after) == 0
      ...
      validation_set_CE = validation_set_CE + CE;
...
av_validation = validation_set_CE/vs_counter;
fprintf(1, '\rAverage Validation CE %.3f\n', av_validation);
[embedding_layer_state, hidden_layer_state, output_layer_state] = ...
  fprop(valid_input, word_embedding_weights, embed_to_hid_weights,...
        hid_to_output_weights, hid_bias, output_bias);
datasetsize = size(valid_input, 2);
