...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.input_to_hid = model.hid_to_class'*((class_prob - data.targets)/n_training_cases).*hid_output.*(1-hid_output)*data.inputs' + model.input_to_hid .* wd_coefficient;
  ret.hid_to_class = (class_prob - data.targets)*hid_output'/n_training_cases + model.hid_to_class .* wd_coefficient;
end
